"""
GST Filing Status Checker — Selenium Automation
================================================
HOW TO RUN:
  ...
  4. For each client, enter CAPTCHA manually in the Chrome window
  5. Click "Next Client" anytime to skip to the next login
...
<head>
<meta charset="UTF-8">
<title>GST Filing Status Checker</title>
<style>
...
<header style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;">
  <div>
    <h1>🧾 GST Filing Status Checker</h1>
...
<div class="container">
  ...
  <div class="card">
    ...
    <div class="col-fmt">
      ...
      <span>GST Portal User ID</span> <span>GST Portal Password</span>
      <span>Client Name</span> <span>GSTIN</span>
    </div>
    <br>
  ...
  <div class="card" id="progress-section">
    ...
    <p class="note" style="margin-bottom:10px">
      ⚠️ Watch the Chrome window — enter CAPTCHA when prompted.<br>
...
function renderResults(rows) {
  ...
  html += '<tr><th class="left" rowspan="2">User ID</th><th class="left" rowspan="2">GSTIN</th><th class="left" rowspan="2">Client Name</th>';
  ...
  rows.forEach(row => {
    html += '<tr>';
    html += `<td class="left">${row['User ID'] || ''}</td>`;
...
def download_template():
    try:
        ...
        ws = wb.active
        ws.title = "Clients"
        headers = ["GST Portal User ID", "GST Portal Password", "Client Name", "GSTIN"]
...
def do_login(driver, username, password):
    try:
        ...
        p = driver.find_element(By.ID,"user_pass")
        p.clear(); p.send_keys(password)
        log(f"  ✏️  Credentials filled. Enter CAPTCHA in the browser window...")
        log(f"  ⏳ Waiting up to {CAPTCHA_WAIT}s — click ⏭ Next or ⏸ Stop anytime.")
        elapsed = 0
        while elapsed < CAPTCHA_WAIT:
            if stop_flag: log("  ⏸  Stopped during CAPTCHA."); return "stopped"
            if skip_flag: log("  ⏭  Skipped during CAPTCHA."); return "skipped"
        ...
        log("  ⏳ CAPTCHA timeout."); return "timeout"
    except Exception as e:
        log(f"  ❌ Login error: {e}"); return "error"
...
def fetch_filing_status(driver, gstin, fin_year):
    ...
    try:
        log("  ⏳ Stabilizing GST session...")
        time.sleep(5)

        log("  🔍 Navigating to Search Taxpayer page...")
        driver.get(GSTIN_SEARCH_URL)
        # If GST portal redirected back to login, session expired
        if "login" in driver.current_url.lower():
            log("  ❌ Session expired after login.")
...
# Wait for page to properly load
        ...
        # ── Fill GSTIN ────────────────────────────────────────────────
        inp = WebDriverWait(driver,15).until(EC.presence_of_element_located((By.XPATH,
            "//input[@placeholder='Enter GSTIN/UIN of the Taxpayer' "
            "or contains(@name,'gstin') or contains(@id,'gstin')]"
        )))
        inp.clear(); inp.send_keys(gstin)
        time.sleep(0.5)
        if should_skip(): return _skipped_result()

        # ── Click SEARCH (taxpayer lookup) ────────────────────────────
        log("  🖱️  Clicking SEARCH (taxpayer lookup)...")
        click_button_by_text(driver, "SEARCH")
        time.sleep(4)
        if should_skip(): return _skipped_result()
...
# ─────────────────────────────────────────────
# LOGOUT — called immediately after status scraped
# ─────────────────────────────────────────────
def do_logout(driver):
    try:
        ...
        if btns:
            ...
            raise Exception("Logout link not found")
    except:
        log("  ⚠️  Auto-logout failed — going to login page.")
        driver.get(GST_LOGIN_URL)
        time.sleep(2)
...
def save_report_now():
    ...
    User ID | GSTIN | Client Name | GSTR-3B (12 months) | GSTR-1/IFF (12 months)
    """
    try:
        if not results_data:
            return
        ...
        # ── Row 1: main headers ──
        # Columns: 1=User ID, 2=GSTIN, 3=Client Name,
        #          4-15 = GSTR-3B months, 16-27 = GSTR-1 months
        ws.merge_cells("A1:A2"); ws["A1"] = "User ID"
        ws.merge_cells("B1:B2"); ws["B1"] = "GSTIN"
        ws.merge_cells("C1:C2"); ws["C1"] = "Client Name"
        ws.merge_cells("D1:O1"); ws["D1"] = "GSTR-3B"
        ws.merge_cells("P1:AA1"); ws["P1"] = "GSTR-1/IFF"

        for cell_ref, fill in [("A1",hdr_fill_b),("B1",hdr_fill_b),("C1",hdr_fill_b),
                                 ("D1",hdr_fill_g),("P1",hdr_fill_r)]:
        ...
        for i,m in enumerate(MONTHS_SHORT):
            # GSTR-3B months: cols D-O (4-15)
            c3b = ws.cell(row=2, column=4+i, value=m)
            c3b.font = hdr_font; c3b.fill = hdr_fill_m; c3b.alignment = center; c3b.border = border
            # GSTR-1 months: cols P-AA (16-27)
            cr1 = ws.cell(row=2, column=16+i, value=m)
            cr1.font = hdr_font; cr1.fill = hdr_fill_m; cr1.alignment = center; cr1.border = border

        for c_ref in ["A2","B2","C2"]:
            ws[c_ref].fill   = hdr_fill_b
            ws[c_ref].border = border

        # ── Data rows ──
        for r_idx, row in enumerate(results_data, start=3):
            ws.cell(row=r_idx, column=1, value=row.get("User ID","")).alignment = left
            ws.cell(row=r_idx, column=2, value=row.get("GSTIN","")).alignment   = left
            ws.cell(row=r_idx, column=3, value=row.get("Client Name","")).alignment = left
            for c in range(1,4):
            ...
            for i, month in enumerate(MONTHS):
                ...
                cell3b = ws.cell(row=r_idx, column=4+i, value=val3b)
                ...
                cellr1 = ws.cell(row=r_idx, column=16+i, value=valr1)
        ...
        ws.freeze_panes = "D3"   # Freeze user/gstin/name columns + headers
...
def _run_automation_inner(fin_year, resume_from=0):
    ...
    for col in df.columns:
        cl = col.strip().lower()
        if ("user" in cl and ("id" in cl or "name" in cl)) or cl in ("username","user id","userid"):
            col_map.setdefault("username", col)
        elif "pass" in cl:
            col_map.setdefault("password", col)
        elif "gstin" in cl or "gst no" in cl or "gst number" in cl:
            col_map.setdefault("gstin", col)
        elif "client" in cl and "name" in cl:
            col_map.setdefault("client_name", col)

    missing = [k for k in ["username","password","gstin"] if k not in col_map]
    if missing:
    ...
    for idx, row in df.iterrows():
        ...
        if login_result == "success":
            ...
            label = statuses["3B"].get("April","")
            stopped = any(v=="Stopped" for v in statuses["3B"].values())
            skipped = any(v=="Skipped" for v in statuses["3B"].values())
            result_row["Remarks"] = "Stopped" if stopped else "Skipped" if skipped else "OK"
            # ── IMMEDIATE LOGOUT after scraping ──
            log("  🔓 Logging out immediately...")
            do_logout(driver)
            if stopped:
                stop_flag = True

        elif login_result == "stopped":
            for ms in MONTHS_SHORT:
                result_row[f"3B_{ms}"] = "Stopped"
                result_row[f"R1_{ms}"] = "Stopped"
            result_row["Remarks"] = "Stopped during login"
            stop_flag = True

        elif login_result == "skipped":
            for ms in MONTHS_SHORT:
                result_row[f"3B_{ms}"] = "Skipped"
                result_row[f"R1_{ms}"] = "Skipped"
            result_row["Remarks"] = "Skipped during login"

        else:
            for ms in MONTHS_SHORT:
                result_row[f"3B_{ms}"] = "Login Failed"
                result_row[f"R1_{ms}"] = "Login Failed"
            result_row["Remarks"] = f"Login: {login_result}"
        ...
        if stop_flag:
            save_session(idx + 1, fin_year, progress["done"], len(df), results_data)
            break
...
def _make_row(idx, name, gstin, user_id, fin_year, status):
    row = {"User ID":user_id,"GSTIN":gstin,"Client Name":name,
           "Financial Year":fin_year,"Processed At":"—","Remarks":status}
    for ms in MONTHS_SHORT:
        row[f"3B_{ms}"] = status
        row[f"R1_{ms}"] = status
    return row
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*52}")
    print(f"  GST Filing Status Checker")
